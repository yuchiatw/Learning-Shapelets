...
from src.learning_shapelets_full_transformer import LearningShapelets
...
def parse_args(configuration_path = "test.yaml"):
    ...
    parser.add_argument('--num_layers', type=int, default=config.get('num_layers', 4), help='Number of layers for the transformer model.')
    ...
    return args
def train(index, configuration_path = "./test1.yaml"):
    ...
    eval_accuracy(model, x_test, y_test)
...
if __name__ == "__main__":
    ...
    #     start = time.time()
    #     train(index = i, configuration_path=config_path)
    #     end = time.time()
    #     elapsed = end - start
