...
def train(index, configuration_path = "/ECG200/test.yaml"):
    ...
    shapelets_size_and_len = {
        ...
        # int(size_ratio_list[1]* len_ts): int(num_shapelets), 
        # int(size_ratio_list[2]* len_ts): int(num_shapelets), 
        # int(size_ratio_list[3]* len_ts): int(num_shapelets), 
    }
    ...
    # fig.savefig(os.path.join(output_dir, f'loss.png'))
    # plt.close()
    ...
    #     fig_ax1.plot(x[i, 0], color='black', alpha=0.5)
    #     _, pos = torch_dist_ts_shapelet(x[i], shapelets[0])
    ...
    #     fig_ax2.plot(x[i, 0], color='black', alpha=0.5)
    #     _, pos = torch_dist_ts_shapelet(x[i], shapelets[1])
    ...
    # fig_ax3.scatter(dist_s1, dist_s2, color=[color[l] for l in y])
    ...
    # plt.savefig(os.path.join(output_dir, f'learning.png'), facecolor=fig.get_facecolor(), bbox_inches="tight")
    # plt.close()


    # output_dir = os.path.join(root, args.dataset)
    ...
    # # Output as CSV file
    # df_x.to_csv(os.path.join(output_dir,'x_test.csv'), index=False)
    
    # # Convert test labels to DataFrame
    # df_y = pd.DataFrame(y, columns=['label'])

    # # Output as CSV file
    # df_y.to_csv(os.path.join(output_dir,'y_test.csv'), index=False)
    ...
    # # Output as CSV file
    # df_shapelets.to_csv(os.path.join(output_dir,'shapelets.csv'), index=False)
    ...
    #         _, pos = torch_dist_ts_shapelet(x_normal[j], shapelets[i])
    ...
    # # Output as CSV file
    # df_pos_record.to_csv(os.path.join(output_dir, 'pos_record.csv'), index=False)
    
    # # Convert shapelet_transform to DataFrame
    # df_shapelet_transform = pd.DataFrame(shapelet_transform)

    # # Output as CSV file
    # df_shapelet_transform.to_csv(os.path.join(output_dir, 'shapelet_transform.csv'), index=False)
