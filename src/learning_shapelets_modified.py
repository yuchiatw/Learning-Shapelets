...
class LearningShapeletsModel(nn.Module):
    ...
    def __init__(self, shapelets_size_and_len, in_channels=1, 
        ...
        self.nhead = nhead
        if self.num_shapelets % nhead != 0:
            print(self.num_shapelets)
            raise ValueError(f"num_shapelets ({self.num_shapelets}) must be divisible by nhead ({nhead})")
        
        # Define the transformer encoder
        d_model = self.num_shapelets  # Embedding dimension should match num_shapelets
        encoder_layers = TransformerEncoderLayer(d_model=d_model, nhead=self.nhead, batch_first=True)
        self.transformer_encoder = TransformerEncoder(encoder_layers, num_layers=number_layers)
    ...
    def forward(self, x, optimize='acc'):
        ...
        """
        x = self.shapelets_blocks(x)  # Output shape: (num_samples, 1, num_shapelets)
        x = self.transformer_encoder(x)  # Output shape: (num_samples, 1, num_shapelets)
        x = x.squeeze(1)  # Remove the added dimension: (num_samples, num_shapelets)
        if optimize == 'acc':
            x = self.linear(x)  # Output shape: (num_samples, num_classes)
        return x
...
class LearningShapelets:
    ...
    def __init__(self, shapelets_size_and_len, loss_func, in_channels=1, num_classes=2,
        ...
        self.model = LearningShapeletsModel(shapelets_size_and_len=shapelets_size_and_len,
                                            ...
                                            to_cuda=to_cuda, nhead=nhead, number_layers=num_layers)
        self.to_cuda = to_cuda
        if self.to_cuda:
            self.model.cuda()
    ...
    def fit(self, X, Y, epochs=1, batch_size=256, shuffle=False, drop_last=False, X_val=None, Y_val=None):  
        """
        Train the model.
        @param X: the time series data set
        @type X: array-like(float) of shape (n_samples, in_channels, len_ts)
        @param Y: the labels of x
        @type Y: array-like or tensor(int) of shape (num_samples,)
        ...
        @type drop_last: bool
        @param X_val: the time series data set
        @type X_val: array-like(float) of shape (n_samples, in_channels, len_ts)
        @param Y_val: the validation target labela
        @type Y_val: array-like or tensor(int) of shape (num_samples,)
        @return: a list of the training losses and validation losses if X_val and Y_val are not None
        @rtype: list(float)
        """
        ...
        if X_val is not None and Y_val is not None:
            if not isinstance(X_val, torch.Tensor):
                X_val = torch.tensor(X_val, dtype=torch.float).contiguous()
            if not isinstance(Y_val, torch.Tensor):
                Y_val = torch.tensor(Y_val, dtype=torch.long).contiguous()
            if self.to_cuda:
                X_val = X_val.cuda()
                Y_val = Y_val.cuda()
        ...
        for epoch in progress_bar:
            ...
            if X_val is not None and Y_val is not None:
                ...
                with torch.no_grad():
                    for x_batch, y_batch in val_dl:
                        ...
                        val_loss += self.loss_func(outputs, y_batch).item()
                val_loss /= len(val_dl)
            ...
            if X_val is not None and Y_val is not None:
                if not self.use_regularizer:
                    progress_bar.set_description(f"Epoch {epoch+1}, Loss: {current_loss_ce}, Val Loss: {val_loss}")
                else:
                    if self.l1 > 0.0 and self.l2 > 0.0:
                        progress_bar.set_description(f"Epoch {epoch+1}, Loss CE: {current_loss_ce}, Loss dist: {current_loss_dist}, Loss sim: {current_loss_sim}, Val Loss: {val_loss}")
                    else:
                        progress_bar.set_description(f"Epoch {epoch+1}, Loss CE: {current_loss_ce}, Loss dist: {current_loss_dist}, Val Loss: {val_loss}")
            else:
                if not self.use_regularizer:
                    progress_bar.set_description(f"Epoch {epoch+1}, Loss: {current_loss_ce}")
                else:
                    if self.l1 > 0.0 and self.l2 > 0.0:
                        progress_bar.set_description(f"Epoch {epoch+1}, Loss CE: {current_loss_ce}, Loss dist: {current_loss_dist}, Loss sim: {current_loss_sim}")
                    else:
                        progress_bar.set_description(f"Epoch {epoch+1}, Loss CE: {current_loss_ce}, Loss dist: {current_loss_dist}")
