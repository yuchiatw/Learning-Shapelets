...
class ShapeletDiscover():
    ...
    def find_ppi(self, i, l, d):
        ...
        for j in range(len(self.group_train_data_piss[l][i][d])): # for each candidate from this time series
            ...
            ts_ci_pis = self.group_train_data_ci_piss[l][i][d][j] # CE(sc_j)
            # Calculate subdist with all time series
            list_dist = []
            for p in range(len(self.train_data)):
                if p == ts_pos: # if it is the time series where the sc comes from
                    list_dist.append(0)
                else:
        ...
        for l in range(len(self.list_labels)):
            ...
            for i in range(len(train_data)):
                if train_labels[i] == self.list_labels[l]:
                    ...
                    self.group_train_data_pos[l].append(i) # sample ID
                    self.group_train_data_piss[l].append(self.train_data_piss[i]) # SC position pair
                    self.group_train_data_ci_piss[l].append(self.train_data_ci_piss[i])  # CE(sc)
