...
def shapelet_initialization(
    X_train, y_train, config, mode='pips'
):
    ...
    if mode=='pips':
        ...
        if os.path.exists(f'./data/list_shapelets_meta_{ws_rate}_{num_pip}_{num_shapelets_make}.csv'):
            df_shapelets_meta = pd.read_csv(f'./data/list_shapelets_meta_{ws_rate}_{num_pip}_{num_shapelets_make}.csv')
            ...
            df_shapelets_meta.to_csv(f'./data/list_shapelets_meta_{ws_rate}_{num_pip}_{num_shapelets_make}.csv', index=False)
            if num_shapelets > len(df_shapelets_meta):
                list_shapelets_meta = df_shapelets_meta.values
            else:
                list_shapelets_meta = df_shapelets_meta.values[:num_shapelets]
...
def train_ls(
    ...
    if model_mode == 'LT_FCN':
        ...
        window_size = max(shapelets_size_and_len.keys())+2
...
def exp(config, store_results = False):
    '''
    Return: 
...
if __name__=='__main__':
    ...
    # elapsed, results, val_loss = exp(config)
    # print(elapsed)
    # print(results)
    # print(val_loss)
    report = []
    yaml_files = glob.glob("./preterm/yaml_LT_pips/*.yaml")
    ...
    for k in range(1):
        ...
        elapsed_list = []
        for j in range(10):
            elapsed, results, val_loss = exp(yaml_config)
        ...
        print("-----------------")
    output_dir = f"./log/preterm/"
    os.makedirs(output_dir, exist_ok=True)
    save_results_to_csv(report, filename=os.path.join(output_dir, 'LS_Trans_PIPs.csv'))